...
public class DiscountSystem {
	...
	/**
	 * Returns a table of items with their corresponding discounts. The included items will only be items
	 * that is contained in the provided item list.
	...
	public Hashtable<IdentificationNumber, Discount> getDiscounts(List<PurchasedItemInformation> itemList) {
		...
		for (PurchasedItemInformation item : itemList) {
			...
			if (itemDiscounts.containsKey(id)) {
				...
				if(!(discount instanceof QuantityDiscount && item.getQuantity() < ((QuantityDiscount) (discount)).getLimit())) {
